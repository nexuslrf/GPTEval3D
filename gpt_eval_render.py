...
from utils import i23d_tournament
...
def parse_args():
    ...
    parser.add_argument(
        '--n_choices', default=3, type=int,
        help="Number of choices for GPT-4V.")
    parser.add_argument(
        '--multiprocessing', default=False, action='store_true',
        help="Use multiprocessing to speed up the evaluation.")
    parser.add_argument(
        '--vlm_model', default="gpt-4o",
        help="VLM model name.")
...
def score_with_new_tournament(args): 
    ...
    tournament = i23d_tournament.I23DTournament(args.tournament)
    ...
    if osp.exists(osp.join(args.comparisons, "question_methods.json")):
    ...
    results, info = tournament.run_gpt4_judge(
        ...
        args.comparisons, max_round=10, n_choices=args.n_choices, model=args.vlm_model,
...
def score_with_existing_tournament(args):
    ...
    new_method = tournament.methods[method_name]
